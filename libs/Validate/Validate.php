...
class Validate {
    ...
    private function email($element,$regla,$params) {
        if (!preg_match('{^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$}',Obj()->Get->getPost($element)) && Obj()->Get->getPost($element)){
            $this->addMsn($element,$regla);
            return false;
        }else{
            return true;
    ...
    private function url($element,$regla,$params) {
        if (!filter_var(Obj()->Get->getPost($element), FILTER_VALIDATE_URL) && Obj()->Get->getPost($element)) {
            $this->addMsn($element,$regla);
            return false;
        } else {
            return true;
    ...
    private function min($element,$regla,$params) {
        if((int)Obj()->Get->getPost($element) < $params && Obj()->Get->getPost($element)){
    ...
    private function number($element,$regla,$params) {
        if (!filter_var(Obj()->Get->getPost($element), FILTER_VALIDATE_INT) && Obj()->Get->getPost($element)) {
            $this->addMsn($element,$regla);
            return false;
        } else {
            return true;
    ...
    private function decimal($element,$regla,$params) {
        if (!filter_var(Obj()->Get->getPost($element), FILTER_VALIDATE_FLOAT) && Obj()->Get->getPost($element)) {
            $this->addMsn($element,$regla);
            return false;
        } else {
            return true;
    ...
    private function date($element,$regla,$params) {
        $fech = str_replace('-', '*', Obj()->Get->getPost($element));
        ...
        if((!checkdate($mes,$dia,$ani) || (strlen($dia) != 2 || strlen($mes) != 2 || strlen($ani) != 4)) && Obj()->Get->getPost($element)){
            $this->addMsn($element,$regla);
            return false;
        }else{
            return true;
    ...
    private function time($element,$regla,$params) {
        ...
        if(!preg_match($pattern,Obj()->Get->getPost($element)) && Obj()->Get->getPost($element)){
            $this->addMsn($element,$regla);
            return false;
        }else{
            return true;
    ...
    private function maxlength($element,$regla,$params) {
        if(strlen(Obj()->Get->getPost($element)) > $params && Obj()->Get->getPost($element)){
            $this->addMsn($element,$regla,$params);
            return false;
        }else{
            return true;
    ...
    private function range($element,$regla,$params) {
        ...
        if(((int)Obj()->Get->getPost($element) >= $ini && (int)Obj()->Get->getPost($element) <= $fin)){
            return true;
        }else{
    ...
    private function rangelength($element,$regla,$params) {
        ...
        if(strlen(Obj()->Get->getPost($element)) >= $ini && strlen(Obj()->Get->getPost($element)) <= $fin){
            return true;
        }elseif(Obj()->Get->getPost($element)){
            $this->addMsn($element,$regla,$ini,$fin);
            return false;
        }
    }
    ...
    private function equalto($element,$regla,$params) {
        if(Obj()->Get->getPost($element) !== Obj()->Get->getPost($params)){
            $this->addMsn($element,$regla);
            return false;
        }else{
            return true;
    ...
    private function noequalto($element,$regla,$params) {
        if(Obj()->Get->getPost($element) === Obj()->Get->getPost($params) && Obj()->Get->getPost($element)){
            $this->addMsn($element,$regla);
            return false;
        }else{
            return true;
